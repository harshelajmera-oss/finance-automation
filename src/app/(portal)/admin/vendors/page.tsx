...
import type { Client, Profile, Vendor } from "@/lib/supabase/types";

export default async function AdminVendorsPage({ searchParams }: { searchParams: Promise<{ clientId?: string }> }) {
  ...
  const [{ data: clients }, vendorsQuery] = await Promise.all([
    supabase.from("clients").select("*").order("name", { ascending: true }).returns<Client[]>(),
    (() => {
      let q = supabase.from("vendors").select("*, clients ( name, code )").order("created_at", { ascending: false });
      if (params.clientId) q = q.eq("client_id", params.clientId);
      return q.returns<(Vendor & { clients: Pick<Client, "name" | "code"> | null })[]>();
    })(),
  ]);
  ...
  return (
    <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10">
      ...
      <p className="mb-6 text-sm text-slate-500">
        ...
        match an existing one by GSTIN or PAN, within that same client. New vendors need approval
        before their ledger and TDS defaults are treated as settled.
      </p>
          ...
          <tbody>
            {(vendors ?? []).map((v) => (
              <tr key={v.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-2 text-slate-500">{v.clients ? `${v.clients.name} (${v.clients.code})` : "—"}</td>
